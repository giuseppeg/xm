...
function expandFillCollections(tree) {
  match.call(tree, { tag: "fill" }, (node) => {
    ...
    let items = [];
    if (src.includes("*")) {
      items = glob(src, { cwd: root }).map((src) => {
        const srcPath = path.resolve(root, src);
        let permalink = path.parse(srcPath.slice(path.resolve(root).length));
        permalink = path.join(permalink.dir, permalink.name);
        const tree = parseToPostHtml(
          `<fill name="xm:permalink">${permalink}</fill>` +
            fs.readFileSync(path.resolve(root, src))
        );
